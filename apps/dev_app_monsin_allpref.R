...
tabset_panel <- 
  tabsetPanel(
    tabPanel(
      title = "全都道府県",
      h3(textOutput("title_allpref")),
      fluidRow(
        column(width=6, withSpinner( plotOutput("graph_bar",height=800)) ),
        column(width=6, withSpinner( plotOutput("graph_line",height=800)) ),
      )
    ),
    tabPanel(
      title = "性別別",
      h3(textOutput("title_gender")),
      fluidRow(
        column(width=6, plotOutput("graph_bar_male",height=400)),
        column(width=6, plotOutput("graph_line_male",height=400)),
        column(width=6, plotOutput("graph_bar_female",height=400)),
        column(width=6, plotOutput("graph_line_female",height=400)),
      )
    ),
    tabPanel(
      title = "年齢指定",
      h3(textOutput("title_age")),
      fluidRow(
        column(width=6, plotOutput("graph_bar_age",height=400)),
        column(width=6, plotOutput("graph_line_age",height=400)),
      )
    ),
    tabPanel(
      title = "性別・年齢別",
      h3(textOutput("title_age_gender")),
      fluidRow(
        column(width=12, plotOutput("graph_age_gender",height=800))
      )
    ),
    tabPanel(
      title = "表",
      h3(textOutput("title_hyou")),
      fluidRow(
        column(width=6,
