...
med_pref_UI <- function(id){
  ...
  tabset_panel <- 
    tabsetPanel(
      ...
      tabPanel(
        title = "経年変化(都道府県別)",
        h3("都道府県別の医薬品処方回数の経年変化"),
        fluidRow(
          column(width=12, withSpinner( leafletOutput( ns("graph_pref_total_map2"), height = 300) )) 
        ),
        fluidRow(
          column(width=12, withSpinner( plotlyOutput(    ns("graph_group_change"   ), height = 800)))
        )
      ),
      tabPanel(
        title = "都道府県別ランキング",
        fluidRow(
          column(width = 12, withSpinner( tableOutput( ns("ranking_by_pref") )))
        )
      )
    )
...
med_pref_Server <- function(id){
  moduleServer(
    ...
    function(input,output,session) {
      ...
      #選択した医薬品の適切な種類を選択する---------------
      #うまくいかない!どこかで実験すること。
      # observeEvent(target_med(),{
      #   
      #   dat <- dat_value()
      #   if(nrow(dat)>0){
      #     browser()
      #     temp_med_type <- dat$med_type %>% unique()
      #     updateRadioGroupButtons(session=session,
      #                             inputId = "medtype",
      #                             choices = c("内服","外用","注射"),
      #                             selected = temp_med_type[1])  
      #   }
      # })
      ...
      similar_yj_med <- reactive({
        if(nrow(target_med()) == 0){
          ...
          warning(similar_yj_med %>% slice(1:2))
      ...
      dat_similar <- reactive({
        ...
        if(nrow(sim_med)==0){
          ...
          warning("from: dat_similar()")
          warning(res %>% slice(1:2))
      ...
      gen_graph_total_bypref <- function(.gdat, tgt_med, pref_name = NA, title = FALSE, fontsize = 18){
        ...
        if(nrow(gdat2) > 0){
          ...
          gg <- ggplot() +
            geom_text(aes(x=1,y=1,label="医薬品の選択、医薬品種類、外来/入院、\n院内/院外の設定を行ってください。"))+
      ...
      output$graph_total_bypref <- renderPlot({
        ...
        warning(str_c("graph total bypref: ", unlist(input$graph_pref_total_map_shape_click)))
        
        return(gen_graph_total_bypref(dat_value_filtered(), target_med(), pref_name = tgt_pref))
      })
      ...
      output$graph_pref_total_map2 <- renderLeaflet({
        gen_graph_pref_total_map(dat_value_filtered(), target_med())
      })
      
      output$graph_group_change <- renderPlotly({
        if(is.null(input$graph_pref_total_map2_shape_click)){
          tgt_pref = NA
        }else{
          tgt_pref <- get_pref_name_from_click(
            input$graph_pref_total_map2_shape_click, dat_map
          )
          
          dat <- dat_similar()
          tgt_med <- target_med()
          tgt_sim_med <- similar_yj_med()
          
          gg <- gen_graph_group_change(dat, tgt_med, tgt_sim_med, pref_name = tgt_pref)
      ...
      gen_rank_table <- function(dat_sim,tgt_med, add_html=TRUE){
        ...
        hh <- dat2 %>% 
          ...
          mutate(is_selected_tgt = all(
            iyakuhin_code == med$iyakuhin_code,
            yj_code == med$yj_code,
            iyakuhin_name == med$iyakuhin_name
          )) %>% 
          ungroup()
        ...
        hh <- hh %>% 
          mutate(pref = str_c(prefid,":",prefname)) %>% 
          select(pref, iyakuhin,value) %>% 
          group_by(pref) %>% 
          arrange(value) %>% 
          filter(!is.na(value)) %>% 
          mutate(rank = rank(rev(value)))
        
        hh2 <- hh %>% 
          filter(rank <= 3) %>% 
          select(pref, iyakuhin, rank)
        
        hh3 <- hh2 %>% 
          pivot_wider(id_cols = pref, names_from = rank, values_from = iyakuhin) %>% 
          select(`都道府県` = pref, `1位` = `1`, `2位` = `2`, `3位` = `3`) %>% 
          ungroup() %>% 
          arrange(`都道府県`)
      ...
      output$ranking_by_pref <- renderTable({
        dat_sim <- dat_similar()
        if(nrow(dat_sim)==0){
          return(tibble("医薬品を選択"))
        }else{
          return(gen_rank_table(dat_similar(),target_med() ))  
        }
      ...
      output$dlexcel <- downloadHandler(
        ...
        content = function(file){
          dat_sim <- dat_similar()
          if(nrow(dat_sim)==0){
            dlhyou <- tibble("医薬品を選択")
          }else{
            dlhyou <- gen_rank_table(dat_similar(),target_med(), add_html = FALSE )
          }
          
          wb <- createWorkbook()
          addWorksheet(wb,"table")
          writeData(wb,"table",dlhyou)
      ...
      output$dlppt <- downloadHandler(
        filename = function(){
          ...
          fn <- str_c("ppt_pref",nam,"_",Sys.Date(),"_", yjc,".pptx")
        ...
        content = function(file){
          ...
          if(is.null(input$graph_pref_total_map2_shape_click)){
            ...
            g2 <- gen_graph_group_change(dat, tgt_med, tgt_sim_med, pref_name = tgt_pref,fontsize = pptfontsize)
          }
