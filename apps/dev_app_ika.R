...
tabset_panel <- 
  tabsetPanel(
    tabPanel(
      title = "単年度",
      h3(textOutput("title_single")),
      fluidRow(
        column(width=4, withSpinner( plotOutput("graph_total",height=400)) ),
        column(width=8, withSpinner( plotOutput("graph_total_age",height=400)) ),
      ),
      fluidRow(
        column(width=12, withSpinner( plotOutput("graph_total_sex_age", height=400)))
      )
    ),
    tabPanel(
      title = "経年変化",
      h3(textOutput("title_multiple")),
      fluidRow(
        column(width=6, withSpinner( plotOutput("time_male"  ,height=200) ) ),
        column(width=6, withSpinner( plotOutput("time_female",height=200) ) )
      ),
      fluidRow(
        column(width=12, withSpinner( plotOutput("time_precise"  , height=1000)))
      )
    ),
    tabPanel(
      title = "表",
